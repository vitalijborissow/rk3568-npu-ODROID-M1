...
> **Yes — minimal oneshot service required.**

The Armbian 6.18 devfreq core clamps `min_freq`/`max_freq` to the boot frequency (600 MHz) despite correct internal state (scaling=200-1000, QoS=0-S32_MAX, 8 OPPs). Investigated:
- `dev_pm_qos_update_request` from probe and delayed work: succeeds but sysfs still shows 600/600
- `initial_freq=0`: no effect
- Vanilla kernel 6.12→6.18 devfreq.c changes don't explain it; Armbian likely has custom patches
- Manual sysfs writes always work

Solution: `rknpu-devfreq.service` — a 5-line systemd oneshot with 5s delay that writes 200M/1000M to sysfs. Installed by `install.sh`.
...
| E | Fix devfreq clamp in driver (remove systemd service) | Medium | Cleaner boot | Won't fix (Armbian kernel quirk) |
...
| 5 | Devfreq range unlock | Low | ✅ Done (systemd oneshot) |
| 6 | Document /dev/rknpu as recommended path | Medium | Pending |
